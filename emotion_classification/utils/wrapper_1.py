...
def get_cls_f1(output, target):
    ...
    score = f1_score(target, output, average='macro') # option = None
...
def loss_plot(train, val, name, title, n_epochs):
    ax = plt.figure().gca()
    ax.plot(train)
    ax.plot(val)
    plt.ylabel("Loss")
...
def train_model_1(model, train_dataloader, test_dataloader, n_epochs):
    ...
    for epoch_i in range(0, n_epochs):
        ...
        for step, (token_ids, segment_ids, attention_mask, label) in enumerate(tqdm(train_dataloader)):
            ...
            label = label.float().to(device) # [1,0,0,0,0,0,0]
        ...
        for (token_ids, segment_ids, attention_mask, label) in tqdm(test_dataloader):
            ...
            out_neu, label_neu = n_out[:,0], label_ids[:,0]
            out_hap, label_hap = n_out[:,1], label_ids[:,1]
            out_sur, label_sur = n_out[:,2], label_ids[:,2]
            out_ang, label_ang = n_out[:,3], label_ids[:,3]
            out_dis, label_dis = n_out[:,4], label_ids[:,4]
            out_sad, label_sad = n_out[:,5], label_ids[:,5]
            out_fea, label_fea = n_out[:,6], label_ids[:,6]
